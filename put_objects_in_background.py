...
def get_refactored_img(background, image, fac_from, fac_to):
    ...
    image = image.resize((int((int(width/new_fac)/max(facWidth, facHeight))), int(
        (int(height/new_fac))/max(facWidth, facHeight))), Image.Resampling.LANCZOS)
...
def put_objects_in_background(background_folder, input_image_folder, output_folder, iterations, iteration_per_background, max_objects):
    ...
    background_filename_list = random.sample(
        glob.glob(background_folder+'*.jpg'), iterations)
    for background_filename in background_filename_list:
        ...
        for i in range(iteration_per_background):
            ...
            place_object_list = random.sample(
                glob.glob(input_image_folder+'*.png'), random.randrange(1, max_objects))
            paste_image_list = []
            for place_object in place_object_list:
                paste_image_list.append(get_refactored_img(
                    background, Image.open(place_object), 2, 3))
            ...
            background.save(output_folder+"gen_"+str(i)+"_" +
                            os.path.basename(background_filename).strip(".jpg")+".png")
            from itertools import combinations
            assert (all(not is_overlap(l1, r1, l2, r2) for (l1, r1),
                    (l2, r2) in combinations(alread_paste_point_list, 2)))
...
if __name__ == '__main__':
    ...
    put_objects_in_background(args.background_folder, args.input_image_folder,
                              args.output_folder, args.iterations, args.iteration_per_background, args.max_objects)
